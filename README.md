...
- **FreeRDP 3:** The runner uses `xfreerdp3`.
...
`RDP_PASS` is provided to FreeRDP through standard input and is not included in
the client process arguments. The configuration file is not encrypted, so
protect it with `chmod 600` and never commit it. FreeRDP uses trust-on-first-use
certificate verification; a changed certificate is rejected after the first
connection. Remove the locally persisted certificate record only when the
container was intentionally reinstalled.
